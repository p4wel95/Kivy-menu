	...
	def click(self,*args): # ta metoda odpala sie po odpuszczeniu przycisku encodera
		...
		if self.aktywny_ekran == 'main1':
			...
			if counter % self.n == 2:
				...
				thirdWindow.video.source = 'elysium.mkv'
			...
			if counter % self.n == 5:
				...
				counter = 0
				if (anim%12 == 0): sixthWindow.anim.source = 'asd.zip'
				if (anim%12 == 1): sixthWindow.anim.source = 'asd2.zip'
				if (anim%12 == 2): sixthWindow.anim.source = 'asd3.zip'
				if (anim%12 == 3): sixthWindow.anim.source = 'asd4.zip'
				if (anim%12 == 4): sixthWindow.anim.source = 'asd5.zip'
				if (anim%12 == 5): sixthWindow.anim.source = 'asd6.zip'
				if (anim%12 == 6): sixthWindow.anim.source = 'asd7.zip'
				if (anim%12 == 7): sixthWindow.anim.source = 'asd8.zip'
				if (anim%12 == 8): sixthWindow.anim.source = 'asd9.zip'
				if (anim%12 == 9): sixthWindow.anim.source = 'asd10.zip'
				if (anim%12 == 10):sixthWindow.anim.source = 'asd11.zip'
				if (anim%12 == 11):sixthWindow.anim.source = 'asd12.zip'
				print ('anim = ',anim)
				anim +=1
				#sixthWindow.anim = AsyncImage(source='asd.zip', pos_hint={"x": 0, "y": 0.1}, size_hint=(1, 0.9),nocache=True)
				#sixthWindow.floatlayout.add_widget(sixthWindow.anim)
				sixthWindow.anim.anim_delay = 0.01
				return True
		...
		if self.aktywny_ekran == 'third':
			...
			thirdWindow.video.source = 'bunny.mp4'
			self.aktywny_ekran = 'main1'
			counter = 0
			sm.current = 'main'
			return True
		...
		if self.aktywny_ekran == 'sixth':
			...
			if counter % self.n == 0:
				...
				sixthWindow.anim.source = 'lowanim.zip'
				#sixthWindow.anim.reload()
...
class ThirdWindow(Screen):
	...
	def __init__(self, **kwargs):
		...
		self.video = Video(source='piroman5.mkv', pos_hint={"x": 0, "y": 0}, size_hint=(1, 1))
...
class SixthWindow(Screen):
	...
	def __init__(self, **kwargs):
		...
		self.im = CoreImage('ball.jpg')
		print(self.im.anim_available)

		self.anim = AsyncImage(source='lowanim.zip', pos_hint={"x": 0, "y": 0.1}, size_hint=(1, 0.9), nocache=True)
		self.anim.anim_delay = -1
		self.floatlayout.add_widget(self.anim)
